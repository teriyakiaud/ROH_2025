...
bedtools merge -i ./data_files/OCN_hs.bed > ./data_files/Oceania_hs_mergedInt.bed
bedtools merge -i ./data_files/OCN_cs.bed > ./data_files/Oceania_cs_mergedInt.bed
...
```{r}
#final output 16 merged hs
hs_mergedInt <- read.table("./data_files/Oceania_hs_mergedInt.bed") %>%
  `colnames<-`(c("CHR", "POS1", "POS2")) %>%
  mutate(POS1 = POS1+1) #need to add one to match coords back from BED format
   
#final output 19 merged cs
cs_mergedInt <- read.table("./data_files/Oceania_cs_mergedInt.bed") %>%
  `colnames<-`(c("CHR", "POS1", "POS2")) %>%
  mutate(POS1 = POS1+1)
