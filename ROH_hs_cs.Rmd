...
#write to BED format
write.table(Africa_global_split, "./BEDfiles/Africa_ROH_500kb.bed", sep = "\t", row.names = FALSE, col.names = FALSE, quote = FALSE)
write.table(America_global_split, "./BEDfiles/America_ROH_500kb.bed", sep = "\t", row.names = FALSE, col.names = FALSE, quote = FALSE)
write.table(Europe_global_split, "./BEDfiles/Europe_ROH_500kb.bed", sep = "\t", row.names = FALSE, col.names = FALSE, quote = FALSE)
write.table(Central_South_Asia_global_split, "./BEDfiles/Central_South_Asia_ROH_500kb.bed", sep = "\t", row.names = FALSE, col.names = FALSE, quote = FALSE)
write.table(East_Asia_global_split, "./BEDfiles/East_Asia_ROH_500kb.bed", sep = "\t", row.names = FALSE, col.names = FALSE, quote = FALSE)
write.table(Middle_East_global_split, "./BEDfiles/Middle_East_ROH_500kb.bed", sep = "\t", row.names = FALSE, col.names = FALSE, quote = FALSE)
write.table(Island_Southeast_Asia_global_split, "./BEDfiles/Island_Southeast_Asia_ROH_500kb.bed", sep = "\t", row.names = FALSE, col.names = FALSE, quote = FALSE)
write.table(Oceania_global_split, "./BEDfiles/Oceania_ROH_500kb.bed", sep = "\t", row.names = FALSE, col.names = FALSE, quote = FALSE)
```
...
bedtools genomecov -i ${bedfile} -g ${genomefile} -bga

```

### genomecov wrapper, used in conjunction with above chunk
```{bash}
#!/bin/bash
...
./genomecov.sh ./BEDfiles/[input_file]_ROH_500kb.bed hs37d5.genome
...
Africa_bg <- read.table("bedgraph_output/500kb_noVanuatu/Africa_ROH_500kb_bg.tsv") 
Africa_bg_mod <- Africa_bg %>%   
  filter(V1 %in% 1:22) %>%          #filter out just the autosomal
  mutate(V1 = as.numeric(V1)) %>%   #make sure its numeric
  add_row(V1 = 22, V2 = .[nrow(.),3], V3 = .[nrow(.),3], V4 = 0) %>%  #add row so that the final value is the end of chr22 in column 3 since we'll erase column 3 later (no gaps from row to row so this works)
  mutate(Region = "Africa")         #add region name for grouping later

Europe_bg <- read.table("bedgraph_output/500kb_noVanuatu/Europe_ROH_500kb_bg.tsv") 
...
EA_bg <- read.table("bedgraph_output/500kb_noVanuatu/East_Asia_ROH_500kb_bg.tsv")
EA_bg_mod <- EA_bg %>%
  filter(V1 %in% 1:22) %>% 
  mutate(V1 = as.numeric(V1)) %>%
  add_row(V1 = 22, V2 = .[nrow(.),3], V3 = .[nrow(.),3], V4 = 0) %>%
...
ISEA_bg <- read.table("bedgraph_output/500kb_noVanuatu/Island_Southeast_Asia_ROH_500kb_bg.tsv")
ISEA_bg_mod <- ISEA_bg %>%
  filter(V1 %in% 1:22) %>% 
  mutate(V1 = as.numeric(V1)) %>%
  add_row(V1 = 22, V2 = .[nrow(.),3], V3 = .[nrow(.),3], V4 = 0) %>%
...
CSA_bg <- read.table("bedgraph_output/500kb_noVanuatu/Central_South_Asia_ROH_500kb_bg.tsv")
CSA_bg_mod <- CSA_bg %>%
  filter(V1 %in% 1:22) %>% 
  mutate(V1 = as.numeric(V1)) %>%
  add_row(V1 = 22, V2 = .[nrow(.),3], V3 = .[nrow(.),3], V4 = 0) %>%
...
Middle_East_bg <- read.table("bedgraph_output/500kb_noVanuatu/Middle_East_ROH_500kb_bg.tsv")
...
America_bg <- read.table("bedgraph_output/500kb_noVanuatu/America_ROH_500kb_bg.tsv")
...
Oceania_bg <- read.table("bedgraph_output/500kb_noVanuatu/Oceania_ROH_500kb_bg.tsv")
...
### Identify top hotspots per region
...
OCN_plot_lower0.01 <- read.table("./data_files/ROH_500Kb_lower0.01_19_hg19.tsv", header = FALSE, sep = "\t") %>%
  `colnames<-`(c("CHR", "POS1", "POS2")) %>%
  mutate(length_kb = (POS2-POS1)*0.001) %>%  #Kb lengths
  mutate(length_mb = length_kb*0.001)
